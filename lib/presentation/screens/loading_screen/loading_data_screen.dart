    ...
    return BlocConsumer<LoadingDataBloc, LoadingDataState>(
      listener: (context, state) {
        if (state.status == LoadingDataStatus.ready) {
          ...
          context.read<AllMoviesBloc>().add(CreateInitialStateEvent(allMovies: state.allMovies, allGenres: state.allGenres));
      ...
      builder: (BuildContext context, LoadingDataState state) {
        if (state.status == LoadingDataStatus.loading || state.status == LoadingDataStatus.ready) {
