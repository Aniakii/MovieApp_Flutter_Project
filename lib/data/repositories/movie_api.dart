...
class MovieAPI {
  ...
  Future<List<Movie>> getMovies() async {
    ...
    for (int i=1;i<10;i++) {
        ...
        dynamic data = await fetchData(urlString: ApiConstants.moviesURL, pageNumber: i);

        if (data['results'] != null) {
          List<dynamic> movieList = data['results'];

          for (var movieData in movieList) {
            String poster = ApiConstants.posterURL+(movieData['poster_path'] ?? '');
            movies.add(Movie(
              id: movieData['id'] ?? 0,
              ...
              overview: movieData['overview'] ?? 'Unknown',
                ...
                posterImage: poster,
                releaseDate: DateTime.parse(movieData['release_date'] ?? '0000-00-00'),
  ...
  Future<dynamic> fetchData({required String urlString, int pageNumber = 0}) async {
